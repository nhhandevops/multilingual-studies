...
MEASURED WHILE BUILDING v0.8 (2026-08-01): **do NOT use kVietnamese for Sino-Vietnamese readings.** Unihan 17.0 carries 8,306 kVietnamese entries, but coverage over the 3,034 characters in our levelled HSK words is only ~68% — with 電 (điện!), 学, 愛, 兒, 很 simply absent (U+96FB has kCantonese/kDefinition/kFanqie/kHangul and no kVietnamese at all) — and the field mixes Hán-Việt with Nôm readings (nộm, phay, chẻm) with no marker distinguishing them. The [RECOMMENDED] above stands for radicals/variants/strokes; its Vietnamese field does not survive contact with the data. v0.8 uses the kaikki.org Vietnamese extract instead (see the kaikki entries): Vietnamese WORD entries carry a `vi-etym-sino` etymology template attesting "Sino-Vietnamese word from X" — 13,866 attested pairs, of which 8,342 match a zh word we ship. Two traps measured there: (1) the template's numbered args may be COMPONENTS (ngân hàng = {1:銀,2:行}) — the source word is the concatenation of all Han-carrying args, and reading arg 1 alone both misses compounds and mis-attaches derived words; (2) match on the template, never the etymology text — the corpus contains "NON-Sino-Vietnamese reading of…" sentences a text regex happily matches. The attested-word design was chosen over composing per-character readings because a composed reading is not a word: 手机 composes to "thủ cơ", and Vietnamese says điện thoại.
